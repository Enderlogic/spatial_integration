...
def construct_graph_by_feature(adata_omics1, adata_omics2, adata_pse_omics1=None, n_neighbors=20, mode="connectivity",
                               metric="correlation"):
    """Constructing feature neighbor graph according to expresss profiles"""

    if adata_pse_omics1 is None:
        adata_pse_omics1 = []
    feature_graph_omics1 = torch.tensor(kneighbors_graph(adata_omics1.obsm['X_pca'], n_neighbors, mode=mode,
                                                         metric=metric).todense()).nonzero().t().contiguous()
    feature_graph_omics2 = torch.tensor(kneighbors_graph(adata_omics2.obsm['X_pca'], n_neighbors, mode=mode,
                                                         metric=metric).todense()).nonzero().t().contiguous()
    feature_graph_pse_omics1 = []
    for temp in adata_pse_omics1:
        feature_graph_pse_omics1.append(torch.tensor(kneighbors_graph(temp.obsm['X_pca'], n_neighbors, mode=mode,
                                                                      metric=metric).todense()).nonzero().t().contiguous())
    return feature_graph_omics1, feature_graph_omics2, feature_graph_pse_omics1
...
def adjacent_matrix_preprocessing(adata_omics1, adata_omics2, adata_pse_omics1=None, n_neighbors=20):
    ...
    edge_index_feature_omics1, edge_index_feature_omics2, edge_index_pse_omics1 = construct_graph_by_feature(
        adata_omics1, adata_omics2, adata_pse_omics1, n_neighbors=n_neighbors)
    ...
    return edge_index_omics1, edge_index_omics2, edge_index_pse_omics1
