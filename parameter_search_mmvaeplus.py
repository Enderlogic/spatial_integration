...
dataset_list = ['human_lymph_node_rep1']
...
for i in range(int(start_idx * len(pss) / total_idx), int((start_idx + 1) * len(pss) / total_idx)):
    ...
    if 'mouse_spleen' in dataset:
        n_cluster_list = [5, 3]
    elif dataset == 'human_lymph_node_rep1':
        n_cluster_list = [6, 10]
    ...
    for _ in range(10):
        model = MMVAEPLUS(adata_srt, adata_spr, n_neighbors=n_neighbors, learning_rate=learning_rate, epochs=500,
